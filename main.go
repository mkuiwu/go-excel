...
func main() {
	//Imports() // 导入
	//ExportSheet()  // 单sheet导出
	//ExportSheets() // 多sheet导出
	//ExportMap() // map导出
	//ExportTree() // 复杂表头导出
	//ExportHorizontal() // 横向合并单元格导出
	//ExportVertical() // 纵向合并单元格导出
	ExportTemplate() // 模板导出
}
...
func Imports() {
	...
	f, err := excelize.OpenFile("C:\\Users\\Administrator\\Desktop\\1.xlsx")
	if err != nil {
		fmt.Println("文件打开失败")
	}
	importList := []Test{}
	err = excel.ImportExcel(f, &importList, 4, 5)
	if err != nil {
		fmt.Println(err)
	}
...
func ExportSheet() {
	...
	//单个sheet导出
	f, err := excel.NormalDynamicExport("Sheet1", "用户信息", "Id,Email,", true, true, testList, changeHead)
	//f, err := NormalDynamicExport(testList, "Sheet1", "用户信息", "", true, false, changeHead)
	if err != nil {
		fmt.Println(err)
		return
	}
	f.Path = "C:\\Users\\Administrator\\Desktop\\单个sheet导出.xlsx"
